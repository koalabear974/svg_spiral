...
function draw() {
  ...
  let border = (width - sourceWidth)/2
  let bottomPos = 500;
  let colors = [rC1, rC2, rC3, rC4];

  let p1 = [border, bottomPos]
  let p2 = [border - lineArc, bottomPos - sourceHeight];
  let p3 = [border + lineArc + sourceWidth, bottomPos - sourceHeight];
  let p4 = [border + sourceWidth, bottomPos];
  stroke('black');
  noFill();
  strokeWeight(1);
  bezier(...p1, ...p2, ...p3, ...p4);
